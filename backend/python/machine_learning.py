...
grouped_df = merge_df1.groupby('Title').agg({'review/score': 'mean','review/text': 'first', 'image':'first',
                                            'authors': 'first', 'categories': 'first', 'Title':'size'})

grouped_df.columns = ['review/score', 'review/text', 'image', 'authors', 'categories', 'count']
