...
# 서울
wp = requests.get("https://www.daangn.com/region/%EC%84%9C%EC%9A%B8%ED%8A%B9%EB%B3%84%EC%8B%9C")
...
import os
if not os.path.exists('images/se'): #이미지폴더
    os.makedirs('images/se') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/se/seimg' + str(i) + '.jpg' for i in range(len(df))]
