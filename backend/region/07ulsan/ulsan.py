...
# 울산
wp = requests.get("https://www.daangn.com/region/%EC%9A%B8%EC%82%B0%EA%B4%91%EC%97%AD%EC%8B%9C")
...
import os
if not os.path.exists('images/us'): #이미지폴더
    os.makedirs('images/us') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/us/usimg' + str(i) + '.jpg' for i in range(len(df))]

df.to_csv("ulsan.csv", index = False)
