...
# 전남
wp = requests.get("https://www.daangn.com/region/%EC%A0%84%EB%9D%BC%EB%82%A8%EB%8F%84")
...
import os
if not os.path.exists('images/jn'): #이미지폴더
    os.makedirs('images/jn') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/jn/jnimg' + str(i) + '.jpg' for i in range(len(df))]

df

df.to_csv("jeonnam.csv", index = False)
