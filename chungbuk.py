...
# 충북
wp = requests.get("https://www.daangn.com/region/%EC%B6%A9%EC%B2%AD%EB%B6%81%EB%8F%84")
...
import os
if not os.path.exists('images/cb'): #이미지폴더
    os.makedirs('images/cb') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/cb/cbimg' + str(i) + '.jpg' for i in range(len(df))]

df.to_csv("chungbuk.csv", index = False)
