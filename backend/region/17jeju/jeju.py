...
# 제주
wp = requests.get("https://www.daangn.com/region/%EC%A0%9C%EC%A3%BC%ED%8A%B9%EB%B3%84%EC%9E%90%EC%B9%98%EB%8F%84")
...
import os
if not os.path.exists('images/jj'): #이미지폴더
    os.makedirs('images/jj') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/jj/jjimg' + str(i) + '.jpg' for i in range(len(df))]

df

df.to_csv("jeju.csv", index = False)
