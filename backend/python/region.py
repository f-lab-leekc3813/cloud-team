...
def func(item):
    ...
    region = item.select(
        'a > div.card-desc > div.card-region-name')[0].text.strip()
    counts = item.select(
        'a > div.card-desc > div.card-counts')[0].text.strip().split()
    like = counts[1]
    chat = counts[-1]
    di = {'item': item_name, 'price': price,
          'region': region, 'like': like, 'chat': chat}
    return di
...
def croll(data):
    ...
    if data == 'best':
        df.to_sql(name='best', con=engine, if_exists='replace', index=True)
    else:
