...
def func(text,img,user_img):
    ...
    di = {'title':title,'user_name':user_name,'user_region':user_region,'temp':temp,'category':category,
          'd_day':d_day, 'price':price, 'info':info,'like':like,'chat':chat,'view':view,
...
def croll(n):
    ...
    return text,img, user_img

def croll_region(data):
    global element, idx
    r = requests.get(region(data))
    soup = BeautifulSoup(r.text, 'html.parser')
    element = soup.find_all(attrs={'class':'card-link'})
    
    li = []
    for n in range(len(element)):
        text,img,user_img = croll(n)
        li.append(func(text,img,user_img))
