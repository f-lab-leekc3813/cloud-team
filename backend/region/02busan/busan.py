...
# 부산
wp = requests.get("https://www.daangn.com/region/%EB%B6%80%EC%82%B0%EA%B4%91%EC%97%AD%EC%8B%9C")
...
import os
if not os.path.exists('images/bs'): #이미지폴더
    os.makedirs('images/bs') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/bs/bsimg' + str(i) + '.jpg' for i in range(len(df))]

df.to_csv("busan.csv", index = False)
