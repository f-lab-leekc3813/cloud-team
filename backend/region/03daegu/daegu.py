...
# 대구
wp = requests.get("https://www.daangn.com/region/%EB%8C%80%EA%B5%AC%EA%B4%91%EC%97%AD%EC%8B%9C")
...
import os
if not os.path.exists('images/dg'): #이미지폴더
    os.makedirs('images/dg') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/dg/dgimg' + str(i) + '.jpg' for i in range(len(df))]

df

df.to_csv("daegu.csv", index = False)
