...
def croll(n):
    link = 'https://www.daangn.com'+part[n].find('a')['href']
    r = requests.get(link)
...
def func(text):
    ...
    di = {'name':name, 'region':region, 'temp':temp, 'title_type':title_type, 'mainTitle':mainTitle,
          'm_name':m_name, 'd_day':d_day, 'pay':pay, 'address':address, 'day':day, 'time':time, 'info':info}
    return di
...
r = requests.get(url)
soup = BeautifulSoup(r.text, 'html.parser')
part = soup.find_all(attrs={'class':'ro1t0q1'})
