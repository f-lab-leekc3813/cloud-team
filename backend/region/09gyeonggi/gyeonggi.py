...
# 경기
wp = requests.get("https://www.daangn.com/region/%EA%B2%BD%EA%B8%B0%EB%8F%84")
...
import os
if not os.path.exists('images/gg'): #이미지폴더
    os.makedirs('images/gg') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/gg/ggimg' + str(i) + '.jpg' for i in range(len(df))]

df.to_csv("gyeonggi.csv", index = False)
