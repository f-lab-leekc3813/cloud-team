...
# 경북
wp = requests.get("https://www.daangn.com/region/%EA%B2%BD%EC%83%81%EB%B6%81%EB%8F%84")
...
import os
if not os.path.exists('images/gb'): #이미지폴더
    os.makedirs('images/gb') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/gb/gbimg' + str(i) + '.jpg' for i in range(len(df))]

df

df.to_csv("gyeongbuk.csv", index = False)
