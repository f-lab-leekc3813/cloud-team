...
# 인천
wp = requests.get("https://www.daangn.com/region/%EC%9D%B8%EC%B2%9C%EA%B4%91%EC%97%AD%EC%8B%9C")
...
import os
if not os.path.exists('images/ic'): #이미지폴더
    os.makedirs('images/ic') #이미지폴더
img_list = []
...
for i in range(len(img_list)):
    image(i)
df['image'] = ['images/ic/icimg' + str(i) + '.jpg' for i in range(len(df))]

df

df.to_csv("incheon.csv", index = False)
